package com.lr.best.mywidget.view;
...
public class CountDownProgressView extends AppCompatTextView {
    ...
    public CountDownProgressView(Context context, AttributeSet attrs) {
        ...
        if (typedArray != null){
            ...
            //TODO 文字颜色属性
            if (typedArray.hasValue(R.styleable.CountDownProgress_textColor1)){
                //有这个属性索引 获取用户设置的颜色
                textColor = typedArray.getColor(R.styleable.CountDownProgress_textColor1,0);
            }else{
                //没有这个属性索引 默认是白色
                textColor = typedArray.getColor(R.styleable.CountDownProgress_textColor1,Color.parseColor("#ffffff"));
            }
     ...
     *  如果先绘制实心圆在绘制文字在绘制空心圆。这时你会发现文字是看不到了，被覆盖了。
     * @param canvas
     */
    @Override
