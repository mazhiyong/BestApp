...
public class CartLocalData {

	private SQLiteDatabase db;

	private static CartLocalData sqliteDBHelp;
	private String dbPath = MbsConstans.DATABASE_PATH+"/"+MbsConstans.DATABASE_NAME;

	private CartLocalData(){
		
	}
	public static CartLocalData getInstance(){
		if (sqliteDBHelp == null) {
			sqliteDBHelp = new CartLocalData();
		}
		return sqliteDBHelp;
	}
	...
	public List<Map<String, Object>> selectDB(){
		...
		openDb();
		if (db.isOpen()) {
			Cursor cursor = db.rawQuery("select * from tb_goods_cart where is_work = 0", null);
			int idIndex=cursor.getColumnIndex("id");
			int goodsIdIndex=cursor.getColumnIndex("goods_id");
			int goodsCountIndex=cursor.getColumnIndex("goods_count");
			int goodsAmountIndex=cursor.getColumnIndex("goods_price");
	...
	public void insertDB(int goodsId,int goodsCount,String goodsAmount){
		openDb();
		if (db.isOpen()) {
			ContentValues values=new ContentValues();
			values.put("goods_id", goodsId);
			values.put("goods_count", goodsCount);
			values.put("goods_price", goodsAmount);
			values.put("is_work", 0);
