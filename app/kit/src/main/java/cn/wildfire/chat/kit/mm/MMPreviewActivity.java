...
public class MMPreviewActivity extends Activity {
    ...
    private void previewVideo(View view, MediaEntry entry) {
        ...
        if (entry.getThumbnail() != null) {
            Glide.with(photoView).load(entry.getThumbnail()).into(photoView);
        } else {
            Glide.with(photoView).load(entry.getThumbnailUrl()).into(photoView);
        }
    ...
    private void previewImage(View view, MediaEntry entry) {
        PhotoView photoView = view.findViewById(R.id.photoView);
        if (entry.getThumbnail() != null) {
            Glide.with(MMPreviewActivity.this).load(entry.getMediaUrl())
